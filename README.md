...
**เดโม:** https://oommiemie.github.io/inventory-online/ — กดปุ่มเดียวเพื่อเข้าใช้งาน (ต้นแบบ จำลองการเข้าสู่ระบบผ่าน Provider ID)
...
- ฝั่งขวา: ปุ่มเดียวส่งต่อไปยัง **Provider ID** ของกระทรวงสาธารณสุข · ไม่มีช่องกรอกชื่อผู้ใช้หรือรหัสผ่าน
  แอปจึงไม่เคยเห็นรหัสผ่านของผู้ใช้ · ท้ายการ์ดมีลิงก์ขอสิทธิ์และตัวสลับภาษา
- Session เก็บที่ `localStorage`
- ออกจากระบบได้จากการ์ดผู้ใช้ท้าย sidebar และ hero ของหน้าตั้งค่า

> ต้นแบบ: การส่งต่อไป Provider ID ยังจำลองด้วยการหน่วงเวลาสั้น ๆ แล้วถือว่าเข้าสู่ระบบสำเร็จ
> ของจริงต้องต่อ OAuth/OIDC และแมปสิทธิ์จากหน่วยงานที่ผูกกับบัญชี
...
Mapping ผูก **รายการของหน่วยบริการ 1 รายการ : Master 1 รายการ** และกำหนดจำนวนต่อหน่วย
เฉพาะคู่ (หน่วยบริการ + รายการ) นั้น — หน่วยบริการต่างกันใช้หน่วยเบิกต่างกันได้

หนึ่งรายการเบิกได้มากกว่าหนึ่งหน่วย เช่น เบิกเป็นลังหรือเป็นกล่องก็ได้ แต่ละหน่วยมีจำนวน
ฐานของตัวเอง (`extraUoms`) และหน่วยเดียวกันจะถูกเลือกซ้ำในรายการเดิมไม่ได้

หน้าจอ **ผูกกับ Master** เป็นงานฝั่ง รพ.สต. จึงแสดงเฉพาะบทบาทที่มี `matching` ในเมนู
ปัจจุบันคือเจ้าหน้าที่คลัง รพ.สต. เท่านั้น · หน้าจอผูกกับหน่วยบริการที่ล็อกอินอยู่โดยตรง
ไม่มีตัวเลือกสลับหน่วยงาน

ขั้นตอน: API ส่งรายการเข้ามา (`UNMAPPED`) → เลือก Master + หน่วย (`DRAFT`) →
เสนอ (`PENDING_APPROVAL`) → โรงพยาบาลอนุมัติ (`ACTIVE`) → ใช้เบิกได้

**Auto Mapping** (`autoMapItems` ใน `app/store.ts`) จับคู่รายการที่ยังไม่มี Master
ให้อัตโนมัติจากชื่อรายการและรหัส แล้วเติมหน่วยหลักของ Master ให้ทั้งสองฝั่ง
ผลลัพธ์หยุดที่ `DRAFT` และถูกเลือกไว้ให้เสมอ — คนตรวจก่อนกดเสนอเสมอ
ไม่มีทางข้ามไปคิวอนุมัติของโรงพยาบาลเอง

---

## คลังต้นทางที่ใช้เบิก

เส้นทางเบิกเก็บทั้งเส้น อ่านจากซ้ายไปขวา

```
หน่วยบริการ → คลังใหญ่หน่วยบริการ → คลังย่อยโรงพยาบาล → คลังใหญ่โรงพยาบาล → โรงพยาบาลแม่ข่าย
   org       localWh (SupplyLink)   subWh (SupplyLink)     wh (SupplyLink)      WAREHOUSES[wh].org
```

`Warehouse.kind` แยก `MAIN` / `SUB` และ `Warehouse.parent` บอกว่าคลังย่อยสังกัดคลังใหญ่ใด
ตัวเลือกคลังย่อยจึงถูกกรองตามคลังต้นทางที่เลือกไว้ และการเลือกคลังย่อยจะตั้งคลังใหญ่
ให้เองอัตโนมัติ — สองคอลัมน์นี้ขัดกันไม่ได้ · เปลี่ยนคลังใหญ่แล้วคลังย่อยที่ไม่ได้สังกัด
คลังนั้นจะถูกล้างทิ้ง · ขออนุมัติไม่ได้ถ้ายังไม่เลือกคลังใหญ่ของหน่วยบริการ
...
| ผู้ใช้งาน | บัญชีผู้ใช้ (ข้อมูลติดต่อ · บัญชีที่เชื่อมต่อ · อุปกรณ์) · ผู้ใช้และบทบาท |

บัญชีผู้ใช้แสดงเป็นโหมดอ่านก่อน กด **แก้ไข** จึงเปลี่ยนเป็นฟอร์ม · ชื่อและบทบาทมาจาก
Provider ID จึงแก้ที่นี่ไม่ได้ แก้ได้เฉพาะอีเมลกับโทรศัพท์ · รูปที่บันทึกสะท้อนทันที
ที่ sidebar และ hero ของ dashboard
